...
def read_teams(skip: int = 0,
               ...
               league_id:int = None,
               db: Session = Depends(get_db)):
    teams = crud.get_teams(db, skip=skip, limit=limit, league_id=league_id, team_name=team_name, min_last_changed_date=minimum_last_changed_date)
    return teams

@app.get("/v0/counts/", response_model=schemas.Counts)
def read_counts(db: Session = Depends(get_db)):
    counts = schemas.Counts(league_count=crud.get_league_count(db), team_count=crud.get_team_count(db), player_count=crud.get_player_count(db))
    return counts
